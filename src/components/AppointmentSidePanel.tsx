...
export default function AppointmentSidePanel(props: {
  ...
  const { open, appointment, onClose, onReschedule, busy } = props;

  const initial = useMemo(() => {
    if (!appointment) return null;
    return {
      startLocal: toLocalInputValue(appointment.startTime),
      minutes: String(Math.max(15, Math.round((new Date(appointment.endTime).getTime() - new Date(appointment.startTime).getTime()) / 60000))),
    };
  }, [appointment]);
      ...
      <div className="absolute right-0 top-0 h-full w-full max-w-md bg-white shadow-lg">
        <div className="flex items-start justify-between border-b p-4">
          <div>
            ...
            <div className="mt-1 text-sm text-gray-600">{a ? a.patientName : ""}</div>
          </div>
        ...
        {!a ? (
          <div className="p-4 text-sm text-gray-600">No appointment selected</div>
        ) : (
          <div className="p-4">
            <div className="rounded-lg border p-3 text-sm">
              <div className="font-medium">{a.patientName}</div>
              <div className="mt-1 text-gray-600">{formatDT(a.startTime)} - {formatDT(a.endTime)}</div>
              <div className="mt-1 text-gray-600">{a.type} · {a.status}</div>
              <div className="mt-1 text-gray-600">{a.providerId} · {a.room}</div>
            </div>
              ...
              <div className="mt-3 flex flex-col gap-3">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    const ok = window.confirm("Reschedule this appointment?");
                    if (!ok) return;
